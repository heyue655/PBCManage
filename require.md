...
- **不同状态的操作权限**：
  ...
  - **已通过（已自评）**：显示"查看"和"查看评价"按钮，**不允许变更**（已进入评估阶段）
...
系统集成钉钉工作通知功能，在PBC审批流程的关键节点自动发送消息提醒。支持多组织配置，不同组织使用独立的钉钉应用。
...
- **配置方式**：
  - 每个用户在"人员管理"中可选择所属组织
  - 系统根据用户所属组织自动选择对应的钉钉应用配置
  - 各组织的Access Token独立管理和缓存
- **字段名称**：`organization`
- **字段类型**：`VARCHAR(50)`，默认值"安恒"
- **字段位置**：`users`表
...
- **配置方式**：
  1. 登录系统进入"人员管理"
  2. 编辑用户信息，首先选择"所属组织"
  3. 在"钉钉用户ID"字段填写对应组织钉钉通讯录中的userid
  4. 如果用户未配置userid，系统会跳过通知，不影响业务流程

**注意**：确保钉钉UserID与所选组织的钉钉通讯录中的用户ID一致！
...
- **Token管理**：各组织独立获取和缓存Access Token，有效期7200秒
- **配置切换**：系统根据用户organization字段自动选择对应的钉钉应用配置

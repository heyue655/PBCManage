...
# 添加组织字段（多组织支持）
mysql -u root -p < database/migrations/005_add_organization.sql

# 创建钉钉应用配置表
mysql -u root -p < database/migrations/006_create_dingtalk_apps.sql

# 导入初始数据
mysql -u root -p < database/seeds/001_init_data.sql

# 导入钉钉应用配置数据
mysql -u root -p < database/seeds/002_dingtalk_apps.sql
```
...
- [x] **钉钉通知集成** 🆕
  - 员工提交审批时通知主管
  - 主管审批（通过/驳回）时通知员工
  - 详见 [钉钉通知文档](./DINGTALK_INTEGRATION.md)
- [x] **多组织支持** 🆕
  - 支持多个组织的用户管理（安恒、中宇华兴）
  - 不同组织使用独立的钉钉应用配置
  - 钉钉通知根据用户所属组织自动切换配置
- [x] **钉钉应用数据库管理** 🆕
  - 钉钉应用配置存储在数据库中，无明文敏感信息
  - 经理及以上角色可管理钉钉应用配置
  - 支持动态添加、编辑、删除、启用/禁用钉钉应用
...
### 钉钉应用接口
- `GET /api/dingtalk-apps` - 获取所有钉钉应用配置（经理及以上）
- `GET /api/dingtalk-apps/:id` - 获取单个钉钉应用配置（经理及以上）
- `POST /api/dingtalk-apps` - 创建钉钉应用配置（经理及以上）
- `PUT /api/dingtalk-apps/:id` - 更新钉钉应用配置（经理及以上）
- `PUT /api/dingtalk-apps/:id/toggle` - 切换启用状态（经理及以上）
- `DELETE /api/dingtalk-apps/:id` - 删除钉钉应用配置（经理及以上）
...
### 目标排序规则
在所有目标列表展示的地方（我的PBC、审核管理、团队目标查看、查看反馈等），目标会按照以下规则自动排序：
1. **首先按目标类型优先级排序**：
   - 业务目标（优先级最高）
   - 组织与人员管理&团队建设（优先级中等）
   - 个人能力提升（优先级最低）
2. **同类型内按权重倒序排列**：权重高的排在前面
3. **排序目的**：
   - 便于评审时优先关注重要的业务目标
   - 清晰展示目标重要性层次
   - 提升评审和查看效率
...
5. **筛选功能**：支持按员工姓名、周期、部门筛选
6. **展示方式**：
   - 分组显示，点击"查看目标"按钮弹窗查看该员工该周期的目标明细
   - 采用卡片式布局，每个目标独立展示
   - 卡片包含：目标序号、名称、类型、状态、权重、描述、实现举措、评价标准、创建时间
   - **实现举措**：业务目标和组织与人员管理&团队建设显示，个人能力提升不显示（因为能力提升用"完成时间"和"衡量标准"）
   - 卡片式布局便于快速浏览和评审，无需左右滚动
7. **自评查看**：员工提交自评后，主管可以点击"查看自评"按钮查看和评价
8. **功能入口**：在系统菜单中"团队目标"菜单项

### 审核管理规则
1. **按员工分组显示**：审核页面按员工和周期分组展示待审核目标
2. **批量审批**：每个员工组提供"批量通过"和"批量驳回"按钮
3. **权重验证**：批量通过时自动验证权重总和是否为100%
4. **查看目标详情**：点击"查看"按钮弹窗展示该员工的所有目标详情
   - 采用卡片式布局，每个目标一张卡片
   - 卡片显示：目标序号、名称、类型、权重、描述、实现举措、评价标准
   - **实现举措**：业务目标和组织与人员管理&团队建设显示，个人能力提升不显示
   - 业务目标额外显示：不可接受标准（红色）、达标标准（蓝色）、卓越标准（绿色）
   - 一眼可看全，无需左右滑动，便于快速评审
